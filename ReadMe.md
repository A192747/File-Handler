...
Решение задачи включает в себя несколько слоев: контроллер, сервис, репозиторий и маппер. </br>
Каждый из этих слоев выполняет определённую функцию, что обеспечивает гибкость и модульность приложения.
* **Контроллер**:
Принимает HTTP запросы и отправляет ответы в формате JSON.
Обрабатывает входящие данные и передает их сервису для выполнения бизнес-логики.
* **Маппер**:
  Преобразует данные между контроллером и сервисом.
  Из Dto в Entity и наоборот.
* **Сервис**:
Обрабатывает бизнес-логику, связанную с файлами и их атрибутами.
* **Репозиторий**:
Реализует взаимодействие с хранилищем файлов.
Сохраняет и возвращает файлы и их атрибуты.
В данном случае для более удобного взаимодействия с БД использовался spring-boot-starter-data-jpa</br></br>
...
### API-методы
#### Создание файла:
**Входные данные:** JSON с файлом (в формате base64) и атрибутами (название - title, дата и время отправки - creation_date, краткое описание документа - description).</br>
```json
{
  "title": "string",
  "creation_date": "2024-07-18T09:08:04.085Z",
  "description": "string",
  "file": "string"
}
```
**Выходные данные:** ID созданного файла.</br>
**Схема работы:**
* Контроллер получает JSON и преобразует его в mapper для преобразования Dto в Entity, а затем перезает в передает его сервису. </br>
* Сервис передает данные, которые необходимо сохранить в repository. </br>
* Сервиc обращается к сохраненной сущности и запрашивает у неё id </br>
* Сервис возвращает ID созданного файла контроллеру
* Контроллер возвращает ответ с ID сохраненного файла
#### Получение файла:
**Входные данные:** ID файла.</br>
**Выходные данные:** JSON с файлом (в формате base64) и атрибутами (название - title, дата и время отправки - creation_date, краткое описание документа - description).</br>
**Схема работы:**
* Контроллер получает ID файла и передает его сервису.</br>
* Сервис получает файл из репозитория и возвращает его.</br>
* Mapper преобразует файл в Dto. </br>
* Контроллер возвращает JSON файла с атрибутами</br>
```json
{
  "title": "string",
  "creation_date": "2024-07-18T09:08:04.085Z",
...
При этом специально были созданы различные dto классы:
* CreateFileRequest - dto, который получает контроллер для создания и сохранения нового файла
* FileResponse - dto, который возвращается при GET запросе на получение файла по ID
* PageFilesResponse - dto, включающий в себя List\<FileResponse> для возврата пользователю страницы с файлами

Решено было использовать различные dto классы для того, чтобы при изменении требований было изменено минимальное кол-во сущностей

#### Получение страницы с файлами
**Входные данные (необязательные):**
* **page:** номер страницы (по умолчанию 1)
* **limit:** количество файлов на странице (по умолчанию 5, ограничено от 1 до 100)
* **sortDirection:** направление сортировки (по умолчанию DESC)</br>

**Схема работы:**
* Контроллер получает параметры page, limit и sortDirection 
* Создается объект Sort с указанным направлением и полем для сортировки. (Поле сортировки можно задать в application.yml, но по умолчанию испольюуется дата создания файла)
* В сервис передается объект PageRequest с указанными параметрами (page, limit, sort).
* Сервис передает объект PageRequest в repository, а в ответ получает Page\<File>
* Сервис возвращает данный Page\<File>, который потом преобразуется при помощи Mapper в объект PageFilesResponse
* Контроллер возвращает клиенту ответ PageFilesResponse

##### Исключительная ситуация
Если переданный номер страницы больше, чем количество всех страниц, то происходит перенаправление клиента на последнюю страницу.

**К примеру:**
Всего 3 страницы, а пользователь запрашивает 4-ую.
В таком случае метод вернет ответ с кодом 302 (Found) и перенаправит на URL с page=3, 
что позволит клиенту перейти на последнюю доступную страницу.
...
<span style="color:pink">В случае неправильного запроса:</span>
```http request
GET http://localhost:8081/api/v1/files?page=-1&limit=101&sortDirection=DESC
```
``` json
{
    "errors": [
        {
            "message": "getAllByPageAndLimit.limit - must be less than or equal to 100"
        },
        {
            "message": "getAllByPageAndLimit.page - must be greater than or equal to 1"
        }
    ],
    "timestamp": "2024-07-18T08:38:08.857+00:00"
}
```
